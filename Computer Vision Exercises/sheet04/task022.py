...
def get_derivs(Im):
    ...
    dx,dy,dxx,dyy,dxy = np.zeros(Im.shape), np.zeros(Im.shape), np.zeros(Im.shape), np.zeros(Im.shape), np.zeros(Im.shape)

    for i in range(1, Im.shape[0]-1):
        for j in range(1, Im.shape[1]-1):
            dx[i,j] = (1/2) * abs(Im[i+1,j] - Im[i-1,j])
            dy[i,j] = (1/2) * abs(Im[i,j+1] - Im[i,j-1])
            dxx[i,j] = Im[i+1,j] - 2*(Im[i,j] + Im[i-1,j])
            dyy[i,j] = Im[i,j+1] - 2*(Im[i,j] + Im[i,j-1])
            dxy[i,j] = (1/4) * (Im[i+1,j+1] - Im[i+1,j-1] - Im[i-1,j+1] + Im[i-1,j-1])
...
if __name__ == '__main__':
    ...
    dx, dy, dxx, dyy, dxy = get_derivs(dist)
    e = 10e-4
...
#        if t % plot_every_n_step == 0:
        ...
        ax3.imshow(dist)
        ax3.set_title("Dist", fontsize=22)
